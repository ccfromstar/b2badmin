...
function _getAll (page,limit,callback) {
  var selectSQL = "select a.*,b.name as c_name,u2.name as aname from "+_tablename+" as a LEFT JOIN company as b ON a.company_id = b.id LEFT JOIN user u2 ON a.approve_id = u2.id where a.certified = 2 order by a.registered_at desc limit " + (page-1)*limit + "," + limit;
  ApiModel.query(selectSQL,function(error,obj){
    ApiModel.query("select * from "+_tablename+" where certified = 2" ,function(error,total){
      var utils = new Utils({
      		page:page,
      		limit:limit,
      		obj:obj,
      		total:total,
          content:(!obj|| !obj[0])?"请选择您要搜索的用户":_setFiled(obj),
          _searchKey:true,
          _width:1100
      });
...
function _getAllbyKey (page,limit,key,callback) {
  var selectSQL = "select a.*,b.name as c_name,u2.name as aname from "+_tablename+" as a LEFT JOIN company as b ON a.company_id = b.id LEFT JOIN user u2 ON a.approve_id = u2.id where "+key+" and a.certified = 1 order by a.registered_at desc limit " + (page-1)*limit + "," + limit;
  ApiModel.query(selectSQL,function(error,obj){
...
function _setFiled(obj){
	var result = "";
	result = result + "<thead>"+_setHeader("姓名|公司|email|手机|注册日期|处理日期|审核人|操作")+"</thead><tbody>";
	for(var i in obj){
		result = result + "<tr>";
		//返回视图显示的列
		result = result + _setCol(obj[i].name+"|"+obj[i].c_name+"|"+obj[i].email+"|"+obj[i].mobile_phone+"|"+(new Date(obj[i].registered_at)).Format("yyyy-MM-dd hh:mm:ss")+"|"+(new Date(obj[i].last_updated_at)).Format("yyyy-MM-dd hh:mm:ss")+"|"+(obj[i].aname?obj[i].aname:""));
