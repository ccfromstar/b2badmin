...
module.exports = {
  ...
  updateRecord:function (req, res) {
     var updateSql = "update passenger set name = '"+req.param('name')
  ...
  getinfo: function (req, res) {
    ...
    var selectSql = "select * from passenger where id = " + id;
    ApiModel.query(selectSql ,function(error,obj){
      obj[0].time = (new Date(obj[0].time)).Format("yyyy-MM-dd hh:mm:ss");
      res.send(obj);
    });
  },
...
function _getSearchoption(callback) {
 ApiModel.query("select * from "+_tablename ,function(error,obj){
  ...
  _read = _read + _setOption("进度|a.schedule|"+obj[i].schedule+"|");
  return callback(_read);
 });
}
...
function _getAll (page,limit,c_userid,callback) {
  //console.log("select a.*,b.name as username from "+_tablename+" a,user b where a.user_id = "+c_userid+" and b.id = a.user_id order by a.time desc limit "+(page-1)*limit+","+limit);
  //console.log("select id from "+_tablename +" a where a.user_id = "+c_userid);
  ApiModel.query("select a.*,b.name as username from ?? a,user b where a.user_id = "+c_userid+" and b.id = a.user_id order by a.time desc limit ?,?",[_tablename,(page-1)*limit,limit],function(error,obj){
    ApiModel.query("select id from "+_tablename +" a where a.user_id = "+c_userid,function(error,total){
        var utils = new Utils({
          ...
          obj:obj,
          total:total,
          content:(!obj|| !obj[0])?"没有我的客户信息":_setFiled(obj),
          _searchKey:true
        });
...
function _getAllbyKey (page,limit,key,c_userid,callback) {
  ApiModel.query("select a.*,b.name as username from "+_tablename+" a,user b where b.id = a.user_id and a.user_id = "+c_userid+" and "+key+" order by a.time desc limit " + (page-1)*limit + "," + limit ,function(error,obj){
...
function _setFiled(obj){
	var result = "";
	result = result + "<thead>"+_setHeader("录入人|姓名|联系方式|咨询内容|进度|类型|时间|操作")+"</thead><tbody>";
	for(var i in obj){
		result = result + "<tr>";
		//返回视图显示的列
    var _type = obj[i].type==1?"散客":"同行";
		result = result + _setCol(obj[i].username+"|"+obj[i].name+"|"+obj[i].contact+"|"+obj[i].content+"|"+obj[i].schedule+"|"+_type+"|"+(new Date(obj[i].time)).Format("yyyy-MM-dd hh:mm:ss"));
