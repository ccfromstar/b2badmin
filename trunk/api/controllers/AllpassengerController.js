...
function _getAll (page,limit,callback) {
  ApiModel.query("select a.*,b.name as username from ?? a,user b where b.id = a.user_id order by a.time desc limit ?,?",[_tablename,(page-1)*limit,limit],function(error,obj){
...
function _getAllbyKey (page,limit,key,callback) {
  ...
  ApiModel.query("select a.*,b.name as username from "+_tablename+" a,user b where b.id = a.user_id and "+key+" order by a.time desc limit " + (page-1)*limit + "," + limit ,function(error,obj){
...
function _setFiled(obj){
	var result = "";
	result = result + "<thead>"+_setHeader("录入人|姓名|联系方式|咨询内容|进度|类型|时间")+"</thead><tbody>";
	for(var i in obj){
		result = result + "<tr>";
		//返回视图显示的列
