...
function _getAll (page,limit,callback) {
  ApiModel.query("select * from ?? where status_id = 3 and cheap = 1 order by created_at desc limit ?,?",[_tablename,(page-1)*limit,limit],function(error,obj){
    ApiModel.query("select id from "+_tablename + " where status_id = 3 and cheap = 1" ,function(error,total){
        var utils = new Utils({
          page:page,
          limit:limit,
          obj:obj,
          total:total,
          content:(!obj|| !obj[0])?"没有数据":_setFiled(obj),
          _searchKey:true,
          _width:1800
        });
...
function _getAllbyKey (page,limit,key,callback) {
  ApiModel.query("select * from "+_tablename+" where status_id = 3 and cheap = 1 and "+key+" order by created_at desc limit " + (page-1)*limit + "," + limit ,function(error,obj){
    ApiModel.query("select id from "+_tablename+" where status_id = 3 and cheap = 1 and "+key ,function(error,total){
        var utils = new Utils({
          page:page,
          limit:limit,
          obj:obj,
...
function _setFiled(obj){
    var result = "";
    result = result + "<thead>"+_setHeader("操作|产品编号|出发日期|出发城市|邮轮公司|邮轮名称|几天几晚|航区|供应商|产品类型|运营性质|发布人|录入人|发布日期|处理日期|审核人")+"</thead><tbody>";
    for(var i in obj){
        result = result + "<tr>";
        result = result + "<td>";
