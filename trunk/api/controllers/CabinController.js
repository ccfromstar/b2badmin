...
function _getSearchoption(callback) {
 ApiModel.query("select * from "+_tablename ,function(error,obj){
  ...
  _read = _read + _setOption("邮轮|txtShipName|0|");
  _read = _read + _setOption("舱房名称|txtCabinName|0|");
  return callback(_read);
 });
}
...
function _setFiled(obj){
	var result = "";
	result = result + "<thead>"+_setHeader("邮轮|舱房编号|舱房名称|舱房类型|最大可入住人数")+"</thead><tbody>";
	for(var i in obj){
		result = result + "<tr>";
		//返回视图显示的列
		result = result + _setCol(obj[i].txtShipName+"|"+obj[i].txtCabinNo+"|"+obj[i].txtCabinName+"|"+obj[i].txtCabinType+"|"+obj[i].numCanCheckIn);
