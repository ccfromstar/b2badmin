...
function _getAll (page,limit,c_userid,callback) {
  var selectSQL = "select a.*,b.name as c_name,u2.name as aname,d.name as dname from user a LEFT JOIN company b  ON a.company_id = b.id LEFT JOIN user u2 ON a.approve_id = u2.id LEFT JOIN user d ON a.parent_id = d.id where a.sales_id = "+c_userid+" and b.role_seller = 0 and a.certified = 1 order by a.registered_at desc limit " + (page-1)*limit + "," + limit;
  ApiModel.query(selectSQL,function(error,obj){
    ApiModel.query("select a.*,b.name as c_name,u2.name as aname,d.name as dname from user a LEFT JOIN company b  ON a.company_id = b.id LEFT JOIN user u2 ON a.approve_id = u2.id LEFT JOIN user d ON a.parent_id = d.id where a.sales_id = "+c_userid+" and  b.role_seller = 0 and a.certified = 1" ,function(error,total){
      var utils = new Utils({
          page:page,
          limit:limit,
          obj:obj,
...
function _getAllbyKey (page,limit,key,callback) {
  var selectSQL = "select a.*,b.name as c_name,u2.name as aname,d.name as dname from "+_tablename+" as a LEFT JOIN company as b ON a.company_id = b.id LEFT JOIN user u2 ON a.approve_id = u2.id LEFT JOIN user d ON a.parent_id = d.id where "+key+" and a.certified = 1 order by a.registered_at desc limit " + (page-1)*limit + "," + limit;
  console.log(selectSQL);
  ApiModel.query(selectSQL,function(error,obj){
...
function _setFiled(obj){
  var result = "";
  result = result + "<thead>"+_setHeader("姓名|公司|email|手机|注册日期|处理日期|审核人|级别|上级|操作")+"</thead><tbody>";
  for(var i in obj){
    result = result + "<tr>";
    //返回视图显示的列
    result = result + _setCol(obj[i].name+"|"+obj[i].c_name+"|"+obj[i].email+"|"+obj[i].mobile_phone+"|"+(new Date(obj[i].registered_at)).Format("yyyy-MM-dd hh:mm:ss")+"|"+(new Date(obj[i].last_updated_at)).Format("yyyy-MM-dd hh:mm:ss")+"|"+(obj[i].aname?obj[i].aname:"")+"|"+(obj[i].retail_level==0?2:1)+"|"+(obj[i].parent_id?obj[i].dname:""));
