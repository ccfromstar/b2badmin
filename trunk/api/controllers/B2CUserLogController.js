...
function _getAll (page,limit,callback) {
  b2cApiModel.query("select pagename,time,ip from ?? order by time desc limit ?,?",[_tablename,(page-1)*limit,limit],function(error,obj){
...
function _setFiled(obj){
	var result = "";
	result = result + "<thead>"+_setHeader("访问页面|访问IP|访问时间")+"</thead><tbody>";
	for(var i in obj){
		result = result + "<tr>";
		//返回视图显示的列
		result = result + _setCol(obj[i].pagename+"|"+obj[i].ip+"|"+(new Date(obj[i].time)).Format("yyyy-MM-dd hh:mm:ss"));
