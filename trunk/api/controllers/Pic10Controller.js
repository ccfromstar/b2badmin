...
var LIMIT = 10;
var _tablename = "promotional_theme";
...
function _getAll (page,limit,callback) {
  b2cApiModel.query("select * from ?? limit ?,?",[_tablename,(page-1)*limit,limit],function(error,obj){
    b2cApiModel.query("select txtThemeNo from "+_tablename ,function(error,total){
        var utils = new Utils({
          page:page,
          limit:limit,
          obj:obj,
...
function _setFiled(obj){
  var result = "";
  result = result + "<thead>"+_setHeader("促销主题名|名称|图片")+"</thead><tbody>";
  for(var i in obj){
    result = result + "<tr>";
    //返回视图显示的列
    var _pic1 = "";
    if(obj[i].rtfImg){
      var _fn1 = obj[i].rtfImg+".jpg";

    //Tools.copy("C:\\pool\\"+_fn1,"C:\\data\\sales\\"+_fn1);
      _pic1 =_pic1+ "<img style='height:30px' src='"+sails.config.globals.url.service.host+"/images/data/sales/"+obj[i].rtfImg+".jpg'></img>";
    }

   
    result = result + _setCol(obj[i].txtThemeName+"|"+obj[i].rtfImg+"|"+_pic1);
    result = result + "</tr>";
  }
  return result;
}
