...
module.exports = {
  ...
  insert:function (req, res) {
     var insertSql = "insert into log_update (time,text) values ('"+
      req.param('col0')+"','"+
      req.param('col1')+"')";
  ...
  updateRecord:function (req, res) {
     var updateSql = "update log_update set time = '"+req.param('col0')
     +"',text ='"+req.param('col1')
  ...
  getinfo: function (req, res) {
    ...
    var selectSql = "select * from log_update where id = " + id;
    ApiModel.query(selectSql ,function(error,obj){
      obj[0].time = (obj[0].time).Format("yyyy-MM-dd")
      res.send(obj);
    });
  },
  ...
  delinfo: function (req,res) {
    var id = parseInt(req.param('id'));
    var selectSql = "delete from log_update where id = " + id;
    ApiModel.query(selectSql ,function(error,obj){
      res.send("200");
    });  
  },
...
function _getSearchoption(callback) {
 ApiModel.query("select * from "+_tablename ,function(error,obj){
  ...
  //_read = _read + _setOption("姓名|a.name|0|");
  return callback(_read);
 });
}
...
function _setFiled(obj){
	var result = "";
	result = result + "<thead>"+_setHeader("更新日期|更新内容||")+"</thead><tbody>";
	for(var i in obj){
		result = result + "<tr>";
		//返回视图显示的列
   
		result = result + _setCol((obj[i].time).Format("yyyy-MM-dd")+"|"+obj[i].text);
		 result = result + "<td style='width:60px'><button type='button' onclick='editinfo("+obj[i].id+")'  class='btn btn-default btn-xs'>编辑</button></td>";
    result = result + "<td style='width:60px'><button type='button' onclick='delinfo("+obj[i].id+")'  class='btn btn-default btn-xs'>删除</button></td>";
