...
function _getAll (page,limit,callback) {
  var selectSQL = "select a.*,b.departmentName as cdepname,b.name as c_name,u2.name as aname,d.name as dname,s.name as sname from user a LEFT JOIN company b  ON a.company_id = b.id LEFT JOIN user u2 ON a.approve_id = u2.id LEFT JOIN user d ON a.parent_id = d.id LEFT JOIN user s ON a.sales_id = s.id where b.role_seller = 0 and a.certified = 1 order by a.registered_at desc limit " + (page-1)*limit + "," + limit;
  ApiModel.query(selectSQL,function(error,obj){
    ApiModel.query("select a.*,b.name as c_name,u2.name as aname,d.name as dname from user a LEFT JOIN company b  ON a.company_id = b.id LEFT JOIN user u2 ON a.approve_id = u2.id LEFT JOIN user d ON a.parent_id = d.id where b.role_seller = 0 and a.certified = 1" ,function(error,total){
      var utils = new Utils({
          page:page,
          limit:limit,
          obj:obj,
