...
function _setFiled(obj){
	var result = "";
	result = result + "<thead>"+_setHeader("访问人公司|访问人姓名|访问IP|访问时间")+"</thead><tbody>";
	for(var i in obj){
		result = result + "<tr>";
		//返回视图显示的列
		result = result + _setCol(obj[i].short_name+"|"+obj[i].name+"|"+obj[i].ip+"|"+(new Date(obj[i].time)).Format("yyyy-MM-dd hh:mm:ss"));
