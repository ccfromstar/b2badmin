...
module.exports = {
  ...
  insert:function (req, res) {
     var insertSql = "insert into cabin_category (ship_id,txtShipNo,txtCabinNo,txtCabinName,txtCabinType,numCanCheckIn) values("+
      req.param('ship_id')+",'"+
      req.param('txtShipNo')+"','"+
      req.param('txtCabinNo')+"','"+
      req.param('txtCabinName')+"','"+
      req.param('txtCabinType')+"',"+
      req.param('numCanCheckIn')+")";
  ...
  _getShipOption: function (req,res) {
     var sql1 = "select * from cruise_ship";
     ApiModel.query(sql1 ,function(error,obj){
        var _read = "<option value='-'>请选择</option>";
        for(var i in obj){
          _read = _read + "<option value='"+obj[i].id+"@"+obj[i].txtShipNo+"'>"+obj[i].txtShipName+"</option>";
        }
        res.send(_read);
     });
  }
...
function _getSearchoption(callback) {
 ApiModel.query("select * from "+_tablename ,function(error,obj){
  ...
  _read = _read + _setOption("公司名称|txtCompanyName|0|");
  _read = _read + _setOption("船名称|txtShipName|0|");
  return callback(_read);
 });
}
...
function _getAll (page,limit,callback) {
  ApiModel.query("select * from ?? order by id asc limit ?,?",[_tablename,(page-1)*limit,limit],function(error,obj){
...
function _getAllbyKey (page,limit,key,callback) {
  ...
  ApiModel.query("select * from "+_tablename+" where "+key+" order by id asc limit " + (page-1)*limit + "," + limit ,function(error,obj){
    ApiModel.query("select id from "+_tablename+" where "+key ,function(error,total){
        var utils = new Utils({
          page:page,
          limit:limit,
          obj:obj,
...
function _setFiled(obj){
  var result = "";
  result = result + "<thead>"+_setHeader("编号|船名|公司名称")+"</thead><tbody>";
  for(var i in obj){
    result = result + "<tr>";
    //返回视图显示的列
    result = result + _setCol(obj[i].txtShipNo+"|"+obj[i].txtShipName+"|"+obj[i].txtCompanyName);
