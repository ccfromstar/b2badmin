...
function _getAll (page,limit,callback) {
  ApiModel.query("select a.*,b.name from "+_tablename+" a,user b where a.user_id = b.id order by a.time desc limit " + (page-1)*limit + "," + limit  ,function(error,obj){
...
function _setCol(col){
	...
	for(var i=0;i<tmp.length;i++){
		r = r + "<td style='min-width:100px'>"+tmp[i]+"</td>";
	}
	return r;
}
