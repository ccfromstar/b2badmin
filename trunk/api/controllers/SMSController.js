...
module.exports = {
  ...
  getPhone: function (req,res) {
    var id = parseInt(req.param('userid'));
    ApiModel.query("select mobile_phone from user where id = "+ id,function(error,obj){
      res.send(obj);
    });
  },
  sendSMS: function (req,res) {
    var content = req.param('content');
    var sendTo = req.param('sendTo');
    var id = req.param('id');
    if(sendTo == "全部"){
        ApiModel.query("select * from user where certified = 1" ,function(error,obj){
            for(var i in obj){
              sms.sendSMS(obj[i].mobile_phone,content);
            }
            res.send("200");
        });
    }else{
      var tmp = sendTo.split("#");
      for(var i=0;i<tmp.length;i++){
        sms.sendSMS(tmp[i],content);
        //创建发送记录
        var sql1 = "insert into smsrecord (user_id,m_telephone,content,time) values ("+id+",'"+tmp[i]+"','"+content+"',now())";
        ApiModel.query(sql1,function(error,obj){});
      }
      res.send("200");
    }
  },
  getContactsOption: function (req,res) {
    _getContactsOption(function (data) {
      res.send(data);
    });
  },
  getContactsOption1: function (req,res) {
    _getContactsOption1(function (data) {
      res.send(data);
    });
  },
  getmyContactsOption1: function (req,res) {
    var id = parseInt(req.param('userid'));
    ApiModel.query("select a.*,b.name as uname from passenger a,user b where a.user_id = b.id and a.user_id="+id ,function(error,obj){
      var _read = "";
      for(var i in obj){
        _read = _read + "<option value='"+obj[i].contact+"'>"+obj[i].name+"</option>";
      }
      res.send(_read);
    });
  },
  getCus: function (req,res) {
    var id = parseInt(req.param('userid'));
    ApiModel.query("select * from customer where user_id="+id ,function(error,obj){
      var _read = "";
      for(var i in obj){
        _read = _read + "<option value='"+obj[i].m_telephone+"'>"+obj[i].name+"["+obj[i].company+"]</option>";
      }
      res.send(_read);
    });
  },
  getCusAll :function (req,res) {
    ApiModel.query("select * from customer_user" ,function(error,obj){
      var _read = "";
      for(var i in obj){
        _read = _read + "<option value='"+obj[i].m_telephone+"'>"+obj[i].name+"["+obj[i].company+"]("+obj[i].username+")</option>";
      }
      res.send(_read);
    });
  }
...
function _getContactsOption(callback) {
 ApiModel.query("select * from user_company where certified = 1" ,function(error,obj){
  var _read = "";
  for(var i in obj){
    _read = _read + "<option value='"+obj[i].mobile_phone+"'>"+obj[i].name+" ["+obj[i].cname+"]</option>";
  }
  return callback(_read);
 });
}

function _getContactsOption1(callback) {
 ApiModel.query("select a.*,b.name as uname from passenger a,user b where a.user_id = b.id" ,function(error,obj){
  var _read = "";
  for(var i in obj){
    _read = _read + "<option value='"+obj[i].contact+"'>"+obj[i].name+" ["+obj[i].uname+"]</option>";
  }
  return callback(_read);
 });
}
