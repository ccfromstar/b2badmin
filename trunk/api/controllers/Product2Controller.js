...
function _getAll (page,limit,callback) {
  ApiModel.query("select * from ?? where status_id = 2 order by created_at desc limit ?,?",[_tablename,(page-1)*limit,limit],function(error,obj){
    ApiModel.query("select id from "+_tablename + " where status_id = 2" ,function(error,total){
        var utils = new Utils({
          page:page,
          limit:limit,
          obj:obj,
...
function _getAllbyKey (page,limit,key,callback) {
  ApiModel.query("select * from "+_tablename+" where status_id = 2 and "+key+" order by created_at desc limit " + (page-1)*limit + "," + limit ,function(error,obj){
    ApiModel.query("select id from "+_tablename+" where status_id = 2 and "+key ,function(error,total){
        var utils = new Utils({
          page:page,
          limit:limit,
          obj:obj,
...
function _setFiled(obj){
    var result = "";
    result = result + "<thead>"+_setHeader("操作|产品编号|产品名称|运营性质|发布人|录入人|发布日期")+"</thead><tbody>";
    for(var i in obj){
        result = result + "<tr>";
        result = result + "<td><button type='button' onclick='setProinfo("+obj[i].id+")' data-toggle='modal' data-target='#ModalRead' class='btn btn-default btn-xs' title='查看产品详情'><span class='glyphicon glyphicon-book'></span></button></td>";
        //返回视图显示的列
