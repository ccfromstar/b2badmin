...
function _setFiled(obj){
	var result = "";
	result = result + "<thead>"+_setHeader("公司名|名称|图片")+"</thead><tbody>";
	for(var i in obj){
		result = result + "<tr>";
		//返回视图显示的列
    var _pic = "<img style='height:30px' src='"+sails.config.globals.url.service.host+"/images/data/shipcompanylogo/"+obj[i].rtfCompanyLogo+"'></img>";
    var _fn = obj[i].rtfCompanyLogo;
    //Tools.copy("C:\\pool\\"+_fn,"C:\\data\\shipcompanylogo\\"+_fn);
		result = result + _setCol(obj[i].txtCompanyName+"|"+obj[i].rtfCompanyLogo+"|"+_pic);
		result = result + "</tr>";
	}
	return result;
}
