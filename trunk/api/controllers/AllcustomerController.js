...
function _getAll (page,limit,c_userid,callback) {
  ApiModel.query("select * from ?? where col3 >= CURDATE() and col0 != '结束' order by col3 asc limit ?,?",[_tablename,(page-1)*limit,limit],function(error,obj){
    ApiModel.query("select id from "+_tablename + " where col3 >= CURDATE() and col0 != '结束'",function(error,total){
        var utils = new Utils({
          page:page,
          limit:limit,
          obj:obj,
...
function _getAllbyKey (page,limit,key,c_userid,callback) {
  ApiModel.query("select * from "+_tablename+" where  "+key+" order by col3 asc limit " + (page-1)*limit + "," + limit ,function(error,obj){
...
function _setCol(col){
	...
	for(var i=0;i<tmp.length;i++){
		r = r + "<td style='max-width:200px;min-width:80px'>"+tmp[i]+"</td>";
	}
	return r;
}
...
function _setFiled(obj){
	var result = "";
	result = result + "<thead>"+_setHeader("状态|地区|更新日期|航次日期|航线|直客价|结算价|收客说明|供应商|录入人")+"</thead><tbody>";
  for(var i in obj){
		result = result + "<tr>";
		//返回视图显示的列
    //result = result + "<td style='width:60px'><button type='button' onclick='editinfo("+obj[i].id+")'  class='btn btn-default btn-xs'>编辑</button></td>";
    
		result = result + _setCol(obj[i].col0+"|"+obj[i].col1+"|"+obj[i].col2+"|"+obj[i].col3+"|"+obj[i].col4+"|"+obj[i].col5+"|"+obj[i].col6+"|"+obj[i].col7+"|"+obj[i].col8+"|"+obj[i].name);
    result = result + "</tr>";
	}
	return result;
}
