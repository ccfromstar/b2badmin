...
var LIMIT = 10;
var _tablename = "log_product_user_company";
...
function _getAll (page,limit,callback) {
  ApiModel.query("select * from ?? order by time desc limit ?,?",[_tablename,(page-1)*limit,limit],function(error,obj){
...
function _getAllbyKey (page,limit,key,callback) {
  ApiModel.query("select * from "+_tablename+" where "+key+" order by registered_at desc limit " + (page-1)*limit + "," + limit ,function(error,obj){
...
function _setFiled(obj){
	var result = "";
	result = result + "<thead>"+_setHeader("产品名称|浏览人公司|浏览人姓名|浏览时间")+"</thead><tbody>";
	for(var i in obj){
		result = result + "<tr>";
		//返回视图显示的列
		result = result + _setCol(obj[i].title+"|"+obj[i].short_name+"|"+obj[i].name+"|"+(new Date(obj[i].time)).Format("yyyy-MM-dd hh:mm:ss"));
