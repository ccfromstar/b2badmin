...
function _getAll (page,limit,callback) {
  var selectSQL = "select a.*,b.name as c_name from "+_tablename+" as a, company as b where a.company_id = b.id and a.certified = -1 order by a.registered_at desc limit " + (page-1)*limit + "," + limit;
  ApiModel.query(selectSQL,function(error,obj){
    ApiModel.query("select * from "+_tablename+" where certified = -1" ,function(error,total){
      var utils = new Utils({
          ...
          obj:obj,
          total:total,
          content:(!obj|| !obj[0])?"没有审核通过的用户":_setFiled(obj)
      });
