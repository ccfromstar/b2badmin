...
var LIMIT = 10;
var _tablename = "smsrecord";

module.exports = {
  getAll: function (req, res) {
  	var page = parseInt(req.param('page'));
    var limit = parseInt(req.param('max'));
    var id = parseInt(req.param('c_userid'));
    page = (page && page > 0) ? page : 1;
    limit = (limit && limit > 0) ? limit : LIMIT;

    _getAll(page,limit,id, function (data) {
      res.send(data);
    });
  },
...
function _getAll (page,limit,id,callback) {
  ApiModel.query("select a.*,b.name from "+_tablename+" a,user b where a.user_id = b.id and a.user_id="+id+" order by a.time desc limit " + (page-1)*limit + "," + limit  ,function(error,obj){
    ApiModel.query("select id from "+_tablename+" where user_id =" + id ,function(error,total){
      var utils = new Utils({
      		page:page,
      		limit:limit,
      		obj:obj,
      		total:total,
      		content:(!obj|| !obj[0])?"没有短信发送记录":_setFiled(obj)
      });
...
function _setFiled(obj){
	var result = "";
	result = result + "<thead>"+_setHeader("发送人|接收人手机|短信内容|发送日期")+"</thead><tbody>";
	for(var i in obj){
		result = result + "<tr>";
		//返回视图显示的列
		result = result + _setCol(obj[i].name+"|"+obj[i].m_telephone+"|"+obj[i].content+"|"+(new Date(obj[i].time)).Format("yyyy-MM-dd hh:mm:ss"));
		result = result + "</tr>";
	}
	return result;
}
