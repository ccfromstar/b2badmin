...
function _getAll (page,limit,callback) {
  var selectSQL = "select a.*,b.name as c_name from "+_tablename+" as a, company as b where a.company_id = b.id and a.certified = 0 order by a.registered_at desc limit " + (page-1)*limit + "," + limit;
  ApiModel.query(selectSQL,function(error,obj){
    ApiModel.query("select * from "+_tablename+" where certified = 0" ,function(error,total){
      var utils = new Utils({
      		page:page,
      		limit:limit,
      		obj:obj,
      		total:total,
          content:(!obj|| !obj[0])?"没有需要审核的用户":_setFiled(obj)
      });
...
function _setFiled(obj){
	var result = "";
	result = result + "<thead>"+_setHeader("姓名|公司|email|手机|注册日期|操作")+"</thead><tbody>";
	for(var i in obj){
		result = result + "<tr>";
		//返回视图显示的列
		result = result + _setCol(obj[i].name+"|"+obj[i].c_name+"|"+obj[i].email+"|"+obj[i].mobile_phone+"|"+(new Date(obj[i].registered_at)).Format("yyyy-MM-dd hh:mm:ss"));
