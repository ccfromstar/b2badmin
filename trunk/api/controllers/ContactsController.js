...
function _getoptionbyId(id,callback) {
  ApiModel.query("select * from user_company where id = "+id ,function(error,obj){
    var _read = "";
  var i = 0;
  ...
  _read = _read + _setOption("公司全称|cname|"+obj[i].cname+"|");
  _read = _read + _setOption("公司简称|cshort_name|"+obj[i].cshort_name+"|");
  _read = _read + _setOption("公司地址|caddress|"+obj[i].caddress+"|");
  _read = _read + _setOption("部门|cdepname|"+obj[i].cdepname+"|");
...
function _getSearchoption(callback) {
 var selectSQL = "select a.*,b.name as c_name,u2.name as aname from user as a LEFT JOIN company as b ON a.company_id = b.id LEFT JOIN user u2 ON a.approve_id = u2.id where a.certified = 1 order by a.registered_at desc";
...
function _update(columnname,oldvalue,newvalue,editorid,id,callback) {
  //判断是公司表还是用户表
  if(columnname=="cname"){
    _tablename = "company";columnname = "name";
  }else if(columnname=="cshort_name"){
    _tablename = "company";columnname = "short_name";
  }else if(columnname=="caddress"){
    _tablename = "company";columnname = "address";
  }else if(columnname=="cdepname"){
    _tablename = "company";columnname = "departmentName";
  }else{
    _tablename = "user";
  }
  if(_tablename == "company"){
  var sql1 = "select company_id from user where id = "+ id;
  ApiModel.query(sql1,function(error,res){
  var updateSQL = "update "+_tablename+" set "+ columnname + "='" + newvalue + "' where id = " + res[0].company_id;
...
function _getAll (page,limit,callback) {
  var selectSQL = "select a.*,b.departmentName as cdepname,b.name as c_name,u2.name as aname,d.name as dname,s.name as sname from user as a LEFT JOIN company as b ON a.company_id = b.id LEFT JOIN user u2 ON a.approve_id = u2.id LEFT JOIN user d ON a.parent_id = d.id LEFT JOIN user s ON a.sales_id = s.id where a.certified = 1 order by a.registered_at desc limit " + (page-1)*limit + "," + limit;
  console.log(selectSQL);
  ApiModel.query(selectSQL,function(error,obj){
    console.log("select * from user where certified = 1" );
    ApiModel.query("select * from user where certified = 1" ,function(error,total){
      console.log("2");
      var utils = new Utils({
          page:page,
          limit:limit,
...
function _getAllbyKey (page,limit,key,callback) {
  var selectSQL = "select a.*,b.departmentName as cdepname,b.name as c_name,u2.name as aname,d.name as dname from user as a LEFT JOIN company as b ON a.company_id = b.id LEFT JOIN user u2 ON a.approve_id = u2.id LEFT JOIN user d ON a.parent_id = d.id where "+key+" and a.certified = 1 order by a.registered_at desc limit " + (page-1)*limit + "," + limit;
  console.log(selectSQL);
  ApiModel.query(selectSQL,function(error,obj){
    var selectSQL1 = "select a.*,b.name as c_name,u2.name as aname from user as a LEFT JOIN company as b ON a.company_id = b.id LEFT JOIN user u2 ON a.approve_id = u2.id where "+key+" and a.certified = 1 order by a.registered_at desc";
...
function _setCol(col){
  ...
  for(var i=0;i<tmp.length;i++){
    r = r + "<td style='max-width:200px'>"+tmp[i]+"</td>";
  }
  return r;
}
...
function _setFiled(obj){
  var result = "";
  result = result + "<thead>"+_setHeader("操作|姓名|微信|公司|部门|email|手机|注册日期|处理日期|审核人|对口销售|级别|上级")+"</thead><tbody>";
  for(var i in obj){
    result = result + "<tr>";
    //返回视图显示的列
    var xs = (obj[i].sname?obj[i].sname:"");
    var p1 = obj[i].weixin_open_id?"√":"";
    ...
    result = result + _setCol(obj[i].name+"|"+p1+"|"+obj[i].c_name+"|"+obj[i].cdepname+"|"+obj[i].email+"|"+obj[i].mobile_phone+"|"+(new Date(obj[i].registered_at)).Format("yyyy-MM-dd hh:mm:ss")+"|"+(new Date(obj[i].last_updated_at)).Format("yyyy-MM-dd hh:mm:ss")+"|"+(obj[i].aname?obj[i].aname:"")+"|"+xs+"|"+(obj[i].retail_level==0?2:1)+"|"+(obj[i].parent_id?obj[i].dname:""));
   result = result + "</tr>";
  }
  return result;
}
