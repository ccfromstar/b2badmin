  ...
  ***************************************************************************/
  onConnect: function(session, socket) {
    ...
    socket.on('online', function(msg){
      sails.log('User ' + msg.user_id + ' is online now!');

      io.emit('online', {'user_id':msg.user_id});
...
function removeOfflineUser(socket){
  ...
  if ( i >= 0 ) {
    sockets.splice(i, 1);
    if ( onlineUsers[i] ) {
      var user_id = onlineUsers.splice(i, 1);
      //notify who is offline now.
      nodeUtil.log('disconnected user id: ' + user_id);
      io.emit(IO_KEY.OFFLINE, {'user_id':user_id});
    }
  }
}
